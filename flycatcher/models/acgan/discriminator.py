...
class Discriminator(nn.Module):
    def __init__(self, channels: int, n_classes: int, img_size: int):
        ...
        def discriminator_block(
            in_filters: int, out_filters: int, apply_batch_norm: bool = True
        ):
        ...
        self.adv_layer = nn.Sequential(
            nn.Linear(
                in_features=128 * self.downsampled_image_size ** 2, out_features=1
            ),
            nn.Sigmoid(),
        )
        ...
        self.aux_layer = nn.Sequential(
            nn.Linear(
                in_features=128 * self.downsampled_image_size ** 2,
